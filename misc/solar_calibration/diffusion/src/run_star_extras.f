...
! ***********************************************************************
      ...
      integer function extras_check_model(id, id_extra)
         ...
         ! Check for radius
         R = s% photosphere_r
         ! write(*, *) 'photosphere_r', R
         if (tol > -1d99 .and. R < 1-tol) then
             extras_check_model = keep_going
             ! write(*, *) 'log R too low, keep going'
         end if
         if (tol > -1d99 .and. R > 1+tol) then
             extras_check_model = retry
             s% dt = s% dt / 10
             write(*, *) 'log R exceeds limit, backing up'
         end if
         if (tol > -1d99 .and. R >= 1-tol .and. R <= 1+tol) then
             extras_check_model = terminate
             write(*, *) 'Terminating: R = R_sun'
         end if
         
         ! Check for luminosity 
         ! if (extras_check_model == keep_going) then
         !    L = s% photosphere_L
         !    if (tol > -1d99 .and. L < 1-tol) then
         !        extras_check_model = keep_going
         !        ! write(*, *) 'L too low, keep going'
         !    end if
         !    if (tol > -1d99 .and. L > 1+tol) then
         !        extras_check_model = retry
         !        s% dt = s% dt / 10
         !        write(*, *) 'L exceeds limit, backing up'
         !    end if
         !    if (tol > -1d99 .and. L >= 1-tol .and. L <= 1+tol) then
         !        extras_check_model = terminate
         !        write(*, *) 'Terminating: L = L_sun'
         !    end if
         ! end if
         ...
         !if (extras_check_model == keep_going) then
         !    age = s% star_age
         !    if (tol > -1d99 .and. age > 4.572d9+0.004d9) then
         !        extras_check_model = retry
         !        s% dt = s% dt / 10
         !        write(*, *) 'age exceeds limit, backing up'
         !    end if
         !end if
         ...
         ! if (extras_check_model == keep_going) then
         !    Teff = s% Teff
         !    if (tol > -1d99 .and. Teff < 5771.8-0.7) then
         !        extras_check_model = keep_going
         !        ! write(*, *) 'age too low, keep going'
         !    end if
         !    if (tol > -1d99 .and. Teff > 5771.8+0.7) then
         !        extras_check_model = retry
         !        s% dt = s% dt / 10
         !        write(*, *) 'Teff exceeds limit, backing up'
         !    end if
         !    if (tol > -1d99 .and. Teff >= 5771.8-0.7 &
         !                    .and. Teff <= 5771.8+0.7) then
         !        extras_check_model = terminate
         !        write(*, *) 'Terminating: 5771.1 <= Teff <= 5772.5'
         !    end if
         !end if
