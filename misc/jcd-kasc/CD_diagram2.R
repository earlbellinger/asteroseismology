...
pred.dir <- file.path('..', '..', 'regression', 'learn',
...
print(exclude)
pred.dir2 <- file.path('..', '..', 'regression', 'learn',
                      'covs-simulations', 'kages')
for (pred.fname in list.files(pred.dir2)) {
    pred.data <- read.table(file.path(pred.dir2, pred.fname), header=1)
    if (any(pred.data$X_c < 0.01))
        exclude <- c(exclude, strsplit(pred.fname, '.dat')[[1]])
}
print(exclude)
...
kages.dir <- file.path('..', '..', 'regression', 'perturb', 'kages')
kages.fnames <- setdiff(list.files(kages.dir),
                         paste0(exclude, '_perturb.dat'))

#legacy.fnames <- c(legacy.fnames, kages.fnames) 
...
plot_CD_diagram <- function(DF, col.pal=colorRampPalette(
    ...
    plot(NA, type='l', axes=F,
        xlim=range(DF$Dnu)+c(0, 15), #33), 
        ylim=c(0.5, max(DF$dnu)),#range(DF$dnu), #+c(0, 0.5),
        xlab=expression("Large frequency separation"~Delta*nu/mu*Hz),
        ylab=expression("Small separation"~
            delta*nu/mu*Hz))
    ...
    for (mass in masses) {
        ...
        if (mass==5)
            text(track$Dnu[1]-5, #-9, 
                 ...
                 pos=3, labels=expression(M==5), cex=0.66*text.cex)
        else
            text(track$Dnu[1]-0.5, #+0,
                 track$dnu[1]-0.14, #-0.9, 
                 pos=3, labels=mass, cex=0.66*text.cex)
    }
    ...
    for (h1 in h1s) {
        ...
        if (h1 == 0.7) {
            text(vals$Dnu-3, 
                 vals$dnu-0.2, 
                 labels="ZAMS", pos=4, cex=0.66*text.cex)
        } else if (h1 == 0.1) {
            text(vals$Dnu-1.5, 
                 vals$dnu, 
                 labels=expression(X["c"]==0.1), pos=4, cex=0.66*text.cex)
        } else {
            text(vals$Dnu-2, 
                 vals$dnu-0.05, 
                 labels=h1, pos=4, cex=0.66*text.cex)
        }
    }
    ...
    points(137, 9, pch=20, cex=0.1)#,
           ...
           #                  * (length(col.pal)-1)) + 1])
    if (put.legacy.points) {
        fnames <- c(file.path(legacy.dir, legacy.fnames), 
                    file.path(kages.dir, kages.fnames))
        for (fname in fnames) {
            #legacy.data <- read.table(file.path(legacy.dir, legacy.fname), 
            #    header=1)
            legacy.data <- read.table(fname, header=1)
    ...
    if (put.legacy.points) {
        ...
        mtext(expression("Metallicity [Fe/H]"), 4, cex=text.cex, line=4)#2)
    }
    
}

make_plots(plot_CD_diagram, 'CD-pointless', DF=DF, put.legacy.points=F,
           mar=utils.mar+c(0,-1,0,3.5),
           slides_pdf_width=9.339975) 
make_plots(plot_CD_diagram, 'CD-points2', DF=DF, put.legacy.points=T,
           mar=utils.mar+c(0,-1,0,3.5), paper=F, thin=F, short=F,
           slides_pdf_width=9.339975) 
