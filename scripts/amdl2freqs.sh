...
if [ -z ${1+x} ] || [ $1 == '-h' ] || [ $1 == '--help' ]; then
    echo "Converter for AMDL to oscillation mode frequencies using ADIPLS."
    echo "Usage: ./amdl2freqs.sh AMDL OUTPUT NSEL FRQWN IORWN"
    echo
...
## Check that the first input (AMDL file) exists
if [ ! -e "$1" ]; then
    echo "Error: Cannot locate AMDL file $1"
    exit 1
fi

## Pull out the name of the AMDL file
bname="$(basename $1)"
fname="${bname%%.*}-freqs"
