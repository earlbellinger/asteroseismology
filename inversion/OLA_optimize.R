...
source('OLA_invert.R')
source('OLA_plots.R')
parallelStartMulticore(9)#max(1,as.integer(Sys.getenv()[['OMP_NUM_THREADS']])))
...
n_trials <- if(length(args)>5) args[6] else 2
half <- F
k.pair <- u_Y
rs <- seq(0.08, 0.3, 0.01)
...
if (trial_i == 1) ref.mod <- model.list[[5]] 
...
#initial_params <- c(rep(10, 2*length(model.list)), 0.02)
initial_params <- c(rep(10,   2*length(model.list)), 
                    rep(0.02, length(model.list)))
#parscale <- c(rep(10, 2*length(model.list)), 1)
...
inversion.list <- parallelMap(function(model_i) {
    ...
    #inversion.list[[model.names[model_i]]] <- 
    inversion$result
}, model_i <- 1:length(model.list))
names(inversion.list) <- model.names
...
save(inversion.lists, file=paste0("inversion.lists", targ.mode))


make_plots(plot_inversion_lists_mean,
    filename=paste0("inv-lists-optim2", targ.mode), 
