\section{Stellar Structure \& Evolution} %Structure \& Evolution of Low-Mass Stars}
\label{sec:evolution}
\begin{shaded}
...
The following makes heavy use of these works, along with calculations using the stellar evolution code \emph{Modules for Experiments in Stellar Astrophysics} \citep[\textsc{MESA},][%revision 10108
...
%Overshooting extends convective zones beyond their ordinary boundaries and smooths over these discontinuities. 
...
%This collection of processes---of which only a subset is ``canonically'' employed in stellar modelling---has been very successful at explaining stellar occupations of the Hertzsprung-Russell and Color-Magnitude diagrams. %, but also in predicting stellar pulsations. %dynamical phenomena of stars: namely, stellar pulsations. 
%Moreover, at each stage of a star's life, asteroseismic theory can be applied to predict the 
%Moreover, at each stage of a star's life, asteroseismic theory can be applied to determine the 
%Asteroseismic theory, visited in detail in the section following this one, is capable of predicting the character of stellar pulsations according to the internal characteristics of the star at each stage of its life, as well as their corresponding periods---within seconds of their measured values. 
...
\begin{enumerate}%[I]
    ...
    I include the effects of a gravitational field, but I ignore electric and magnetic fields. %, whose presence are similar to that of rotation. 
    ...
    %However, these effects occur in giant stars, and the present thesis is concerned with main-sequence stars. 
    %\item \emph{Stars are stationary on dynamical timescales.} I ignore net velocities and accelerations. This assumption breaks down during core collapse preceding a supernova and in regions of the stellar interior where energy is transported via convection. The latter issue is circumvented ... \emph{\textbf{TODO}: something about convection}
    %\item \emph{Stars are blackbodies in local thermodynamic equilibrium (LTE).} 
    %This allows us to calculate the temperature and other thermodynamic properties of the stellar matter at each point locally, without global considerations. 
    %I ignore net velocities and accelerations, and, for now, pulsations. 
    %This allows us to define a temperature distribution in the stellar interior. %We furthermore assume that the particle velocities are Maxwellian and that the radiation field is a blackbody at that same temperature. 
    %This assumption breaks down near the stellar surface, during core collapse preceding a supernova, and in regions of the stellar interior where energy is transported via convection. 
\end{enumerate}
...
\begin{description}%[leftmargin=.5in]
    ...
    %Let $\epsilon$ be the coefficient of energy generation per unit time and unit mass. 
    ...
    \begin{equation} \label{eq:energy} \boxed{
        ...
        %\epsilon(r)
        %\\ &=
        \epsilon_{\text{nuc}} - \epsilon_\nu + \epsilon_g %C_P \frac{\text{d}T}{\text{d}t} - \frac{\delta}{\rho} \frac{\text{d}P}{\text{d}t} %\notag
    }\end{equation}
    where $\epsilon_{\text{nuc}}$ is the energy generated by nuclear reactions, $\epsilon_\nu$ is the energy lost by neutrinos, and $\epsilon_g$ is the gravitational energy from expansion or compression: %% TODO: CHECK THIS 
    ...
    %The EOS and nuclear energy generation rates are supplied externally. 
    %For the low-mass stars considered here, I use the \emph{Opacity Project at Livermore} EOS \citep[\textsc{OPAL},][]{2002apj...576.1064r} and the rates from the \emph{Nuclear Astrophysics Compilation of Reaction Rates} \citep[\textsc{NACRE},][]{1999NuPhA.656....3A}. 
    ...
    The adiabatic exponents $\boldsymbol\Gamma$, introduced by Chandrasekhar, give these relations as follows: %are related to the structure of the star as follows: 
    \begin{align} 
        \Gamma_1 
        &= 
        \left(
    ...
    \begin{equation} \label{eq:speed-of-sound}
        c = \sqrt{\Gamma_1 u}. 
    \end{equation}
    %where $u=P/\rho$ is the isothermal speed of sound. 
    ...
    %Finally, the third describes how compression influences the local temperature. 
    In an ideal monoatomic gas, the adiabatic exponents all equal $5/3$. %; for pure radiation, $4/3$. %; for pure convection, $2/5$ ($ = 1.2/3$). 
    %Their values throughout the star depend on the local properties of the stellar matter and the assumed EOS. 
    %In an ideal gas, $\Gamma_1 = \Gamma_2 = \Gamma_3 = \frac{5}{3}$. For pure radiation, $\Gamma_1 = \Gamma_2 = \Gamma_3 = \frac{4}{3}$. 

    %\begin{align}
    %    P &= P(\rho, T, \mathbf{X})\notag\\
    %    \rho &= \rho(P,T, \mathbf{X})\\
    %    T &= T(\rho, P, \mathbf{X})\notag
    %\end{align}
    %where $\mathbf{X}$ is the composition of the material. 
    
    %The additional terms account for energy gained or lost due to compression or expansion over time $t$. 
    %Here $C_P$ is the specific heat at constant pressure. 
    %The equation of state (EOS)---which relates pressure, density, and temperature---supplies the last term, with
    %\begin{equation}
    %    \delta = \left( \frac{\delta \ln \rho}{\delta \ln T} \right)_{P,X_i}(r).
    %\end{equation}
    %where the derivative is taken at constant pressure and abundance. 
    %We also need an equation of state (EOS) to relate pressure, density, and temperature. 
    %Nuclear reaction rates come from 
    %where
    %\begin{equation}
    %    \delta 
    %    = 
    %    - \left( \frac{\delta \ln \rho}{\delta \ln T} \right)_{P,X_i}
    %\end{equation}
    ...
    %We define using the Schwarzschild criterion, i.e.
    \fi
\end{description}
...
%The boundary conditions here are, simply enough, 
Here we have %, simply enough, 
\begin{equation}
    m=0,\qquad r=0,\qquad l=0.
\end{equation} 
%There is no mass in an infinitesimally small sphere at this location: $m=0$. 
%at the location $r=0$, or equivalently, $m=0$. 
%Correspondingly, there is nothing to fuse, and so $L(m=0)=0$. 
%If the flow of energy in this region were nonzero, then the stellar luminosity would be infinite. 
%Hence, $l(m=0)=0$. 
%This gives us two boundary conditions. 
The second boundary is at the stellar surface. %, or equivalently, $m=M$. 
...
%I use a standard Eddington gray atmosphere. %, which is a relationship between the temperature and optical depth $\tau$:
...
\end{equation}
following the Stefan-Boltzmann Law %\footnote{ Later on, this equation will give us a first solution to our inverse problem (see Section \ref{sec:scaling}).} 
for blackbody radiation: 
\begin{align} \label{eq:stefan-boltzmann} 
    L &= 4\pi R^2 \sigma T_{\text{eff}}^4
\end{align}
where $\sigma$ is again the Stefan-Boltzmann constant. %, and 
%The total luminosity and surface temperature (i.e., the \emph{effective temperature})
%\begin{align}
%    l(r=R) &= L
%    T(r=R) &= T_{\text{eff}}
%\end{align}
%Both the luminosity at the surface, $l(r=R)=L$, and the temperature at the surface (the \emph{effective temperature}), $T(r=R)=T_{\text{eff}}$, 
%are here both given by 
...
%Now properly equipped with boundary conditions, the equations of stellar structure can be solved. 
%\emph{\textbf{TODO}: atmosphere, boundary conditions}

% that, when solved, give us the profiles of a star. 

%\subsubsection*{A remark on thermodynamics}

\subsubsection*{Stellar Evolution}
For a star to \emph{evolve}, it must change over time. With the exception of one term for the gravitational energy from expansion or compression (Equation~\ref{eq:eps-g}), the equations of stellar structure feature no time derivatives; %\footnote{ This is with exception of the energy equation, which responds to the changing radius.}; 
they describe a static star. 
%The changes in a star can come from 
...
%The equations dictating the evolution of these species are as follows: 
...
\begin{description}
    ...
    %These rates must be supplied externally. A common choice are the NACRE reaction rates \citep{1999NuPhA.656....3A}. 
    ...
    \item[Convection.] 
    %Convection is an efficient mixer; consequently, convective regions are usually chemically homogeneous. 
    ...
    Here I will treat convection using the standard \citet{1958ZA.....46..108B} mixing length theory, which approximates the effects of convection by assuming that convective elements travel to some characteristic length $\ell_m$ before mixing the transported material with their newfound surroundings. %Thus, convective zones are chemically homogeneous. 
    ...
    %Details for calculating the resulting convective temperature gradient $\nabla$ can be found in the aforementioned textbooks. 
    
    Convection is an efficient mixer. %, convective regions are usually chemically homogeneous. 
    ...
    \lr{\begin{equation} \boxed{
        ...
        \right)
        %\frac{\delta}{\delta m} \left(
        %    v_c \cdot 
        %    \ell_{m} \cdot
        %    \frac{\delta X_i}{\delta m}
        %\right)
    }\end{equation}}
...
We will now solve these equations and model the evolution of the stars. %\emph{\textbf{TODO}: Henyey scheme }

%\subsubsection*{Initial Model Composition}
%\noindent Our models need a starting point, which is a given mass and chemical composition. 
%Generally, astronomers think of stars as being composed of three main ingredients: hydrogen ($X$), helium ($Y$), and heavier elements ($Z$), where 
%\begin{equation}
%    X+Y+Z=1.
%\end{equation}
%The mass of the solar photosphere is $X\simeq 71\%, Y\simeq 27\%$. 
%The remainder, $Z$, is of course composed of many elements. 
%In stellar modelling, a pre-defined initial mixture of the elements in $Z$ is usually assumed in accordance with the observed composition of the solar photosphere. 
%I use the solar composition as measured by \citet[GS98,][]{1998SSRv...85..161G} which gives good agreement with helioseismology. 
%Note that there are other and more recent choices, such as those of \citet{2009ARA&A..47..481A}, but these do not agree as well with helioseismology---an issue famously known as the \emph{solar abundance problem}. 
...
%The speed of sound and first adiabatic exponent both increase slightly outward away from the core. 
%The speed of sound %and the first adiabatic exponent are both lower in the direct center of the star than slightly outward. 
%The speed of sound increases slightly outward away from the core due to electron degeneracy before falling by several orders of magnitude. 
%The speed of sound is lower in the direct center than slightly outward due to 
%The increasing away from the core is caused by the slight electron degeneracy of the core, 
% central degeneracy, non-ideal gas, electron degeneracy 
...
\begin{figure}
    ...
    \begin{subfigure}[b]{0.5\linewidth}
        \centering
        \includegraphics[width=\textwidth,keepaspectratio]{figs/pulse/diffs/prof_D-rho.pdf}%d_rho-D_noD.pdf}
    \end{subfigure}\\
    \begin{subfigure}[b]{0.5\linewidth}
        \centering
        \includegraphics[width=\textwidth,keepaspectratio]{figs/pulse/diffs/prof_D-Gamma1.pdf}%d_Gamma1-D_noD.pdf}
    \end{subfigure}%
    \begin{subfigure}[b]{0.5\linewidth}
        \centering
        \includegraphics[width=\textwidth,keepaspectratio]{figs/pulse/diffs/prof_D-Y.pdf}%d_Y-D_noD.pdf}
    \end{subfigure}
...
\begin{figure}
    \centering
    %\includegraphics[width=\textwidth]{chem_ev_sun_surf.pdf}\\
    ...
    %The majority of the evolution through the subgiant and red giant phases is not depicted, as the core composition is there unchanging. 
    \label{fig:chem_ev}} 
%\end{figure}
%\begin{figure}
%    \centering
    \vspace*{0.1cm}
    \input{figs/evol/configurations.tex}
...
\begin{figure}
    \centering
    %\begin{minipage}[b]{0.5\textwidth}\vspace{0pt}\includegraphics[width=\textwidth,trim={0 1.6cm 0 0},clip]{figs/evol/hr-M.pdf}\end{minipage}%
    %\begin{minipage}[b]{0.5\textwidth-1.5cm}\vspace{0pt}\includegraphics[width=\textwidth,trim={1.8cm 1.6cm 0 0.},clip]{figs/evol/hr-alpha.pdf}\end{minipage}\\
    %\begin{minipage}[b]{0.5\textwidth}\vspace{0pt}\includegraphics[width=\textwidth,trim={0 0 0 0},clip]{figs/evol/hr-Y.pdf}\end{minipage}%
    %\begin{minipage}[b]{0.5\textwidth-1.5cm}\vspace{0pt}\includegraphics[width=\textwidth,trim={1.8cm 0 0 0},clip]{figs/evol/hr-Z.pdf}\end{minipage}
