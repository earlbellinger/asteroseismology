...
C     f(2) = a*y_2 - b*y_1 + f
C     WHERE
C     a = (r*rho'+2*rho)/(r*rho)
C     b = 4*pi*G*rho^2/P
C     f = r^2*rho * d/dr ( F / (r^2*rho) )
C     F = (Gamma_1,P + Gamma_1,rho) * K_Gamma1_rho + K_rho_Gamma1
...
C     PAR(2*N+1..3*N) = f(T)
      ...
      F(1)=Y(2)
      F(2)=PAR(J)*Y(2)-PAR(N+J)*Y(1)+PAR(2*N+J)
...
C     BOUNDARY CONDITIONS    Y1(T=T1)=0,  Y1(T=T2)=0
      G(1)=Y1(1)
      G(2)=YN(1)
