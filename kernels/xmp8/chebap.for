C     PROGRAM TO FIND COEFFICIENTS OF RATIONAL FUNCTION APPROXIMATION 
C     IN TERMS OF CHEBYSHEV POLYNOMIALS USING THE KNOWN CHEBYSHEV EXPANSION

      PROGRAM CHEBY
      IMPLICIT REAL*8(A-H,O-Z)
      DIMENSION A(20),WK(100),C(20),IWK(10)

C     EXAMPLE 9.9 : ARC TAN(X)
...
C     COEFFICIENTS OF CHEBYSHEV EXPANSION OF ARC TAN(X)
...
C	To calculate coefficients of rational function Chebyshev approximation
C	using the known coefficients of Chebyshev expansion
C
...
C		of rational function approximation. A(I) is the coefficient
C		of T_I(x) in the denominator, the constant term being 1.
C		A(K+J+1) is the coefficient of T_J(x) in the numerator
C	C : (input) Real array of length M+2K+1 containing the coefficients
C		of Chebyshev expansion. C(I+1) is the coefficient of T_I(x).
C		Coefficient of T_0(x) is C(1)/2
...
C
      ...
      IER=0
      IF(K.GT.0) THEN
C	Setting up the coefficients of matrix of linear equations
C	to calculate the coefficients in the denominator
        DO 2200 I=1,K
          DO 2000 J=1,K
            WK(J,I)=0.5*(C(M+J+I+1)+C(IABS(M+J-I)+1))
2000      CONTINUE
C	The right-hand side vector for linear equations
          A(I)=-C(M+I+1)
2200    CONTINUE
...
C	Calculating the coefficients in the numerator
      DO 3000 I=1,M+1
        AI=C(I)
        DO 2600 J=1,K
