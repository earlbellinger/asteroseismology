C     PROGRAM FOR FINDING EIGENVALUES AND LEFT-EIGENVECTORS OF UNSYMMETRIC MATRIX
     ...
     1          ,INT(30),CWK(300)
C      DATA ((A(I,J),J=1,7),I=1,7)/6,0,0,0,0,1,0, 0,4,0,3D-8,1D-4,2D-4,
C     1   1D-3, 1,10000,7,0,0,-2,20, 0,2D8,0,1,-40000,30000,-4D5,
C     2   -2,-30000,0,1D-4,2,2,40, 0,0,0,0,0,0,0,
C     3   0,1000,0,4D-5,0.1D0,-0.2D0,3/
...
C     FINDING THE LEFT-EIGENVECTORS USING INVERSE ITERATION
...
C     BACK-TRANSFORM THE LEFT-EIGENVECTORS
...
C	Perform back-transformation of a set of left eigenvectors from
C	those of balanced matrix to that for original matrix
C
C	N : (input) Order of matrix
C	LOW, IGH : (input) After balancing only rows and columns from
C		LOW to IGH, since other rows or columns contain isolated
C		eigenvalues
...
C	Required routines : None
      ...
      DO 3000 I=LOW,IGH
C       S=D(I)
C	For back-transforming right eigenvectors use the preceding
C	statement instead of the following one
        S=1/D(I)
...
C	Complex eigenvalue and left eigenvector of a real matrix using inverse iteration
C
...
C		initial approximation to left eigenvector. After execution it
