C     PROGRAM FOR RECURSIVE EVALUATION OF TRIPLE INTEGRAL
C     ADPIN1, ADPIN2 AND KRONR1, KRONR2 ARE COPIES OF ADPINT AND KRONRD
...
C     EXERCISE 6.44 : INTEGRATION OVER ELLIPSOID
...
C     ALP IS THE COORDINATE A IN THE EXERCISE
...
C     THE REQUIRED INTEGRAND IS THE INTEGRAL OVER Y AND Z DIRECTIONS
...
C     STORE THE VALUE OF X IN THE COMMON BLOCK FOR USE BY F1 AND F2
      XX=X
...
C     THE REQUIRED INTEGRAND IS THE INTEGRAL OVER Z DIRECTION

C     LIMITS FOR INTEGRATION IN Z DIRECTION
...
C     STORE THE VALUE OF Y IN THE COMMON BLOCK FOR USE BY F2
      YY=Y

C     INTEGRATE W.R.T. Z TO OBTAIN THE FUNCTION F1
      CALL ADPIN2(F1,A,B,REPS,AEPS,DIF,F2,IER,NPT,NMAX)
      IF(IER.GT.0) STOP 12

C     ACCUMULATE THE NO. OF FUNCTION EVALUATIONS IN THE COMMON BLOCK
      NP=NP+NPT
      END
...
C     THE VALUES OF X AND Y ARE TAKEN FROM THE COMMON BLOCK
C     TAKING ACCOUNT OF SYMMETRY ALONG Y AND Z AXES THE INTEGRAND
C     IS MULTIPLIED BY 4 SINCE THE RANGE OF INTEGRATION IS
C     RESTRICTED TO POSITIVE QUADRANT IN Y-Z PLANE
