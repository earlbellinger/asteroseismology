...
C     SINGULAR VALUE DECOMPOSITION IN 3 DIMENSIONS
...
C     FUNCTION WITH RANDOM ERROR TO GENERATE INPUT DATA
      ...
      F(Y,Y1,Y2)=((231*Y*Y1-315)*Y*Y2+105)*Y1*Y2-5+RANGAU(IS)*1.D-3
...
C    For fit in 3 dimensions the number of basis functions should be
C    M0**3 where M0 is number of basis functions along each dimension.
...
C	  To calculate the set of basis functions in 3 dimensions
C     These are the monomials in 3 D in natural order

      SUBROUTINE PHI(N,X,F)
      IMPLICIT REAL*8(A-H,O-Z)
      DIMENSION F(N),X(3)
 
C	The number N is assumed to be a cube of integer
