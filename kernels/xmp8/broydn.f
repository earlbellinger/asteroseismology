C     PROGRAM TO SOLVE A SYSTEM OF NONLINEAR EQUATIONS USING BROYDEN'S METHOD
...
C	Real root of a system of nonlinear equations using Broyden's method
C
...
C			accuracy
C	WK : Real array of length MAX(2*NP*NP,NP*(NP+4)) used as scratch space
C	IWK : integer array of length NP used as scratch space
C
C	SUBROUTINE FCN(NP,X,F) must be supplied by the user.
C	Here X and F are real arrays of length NP containing the value of
C	vector function at point X.
...
C	Generating initial approximation to the Jacobian
      DO 2000 IP=1,NP
        X1=X(IP)
        DK=0.01D0*ABS(X1)
...
C	For use with subroutine BROYDN
C
C	FCN : (input) Name of the subroutine to calculate the vector function
C	NP : (input) Number of variables and equations to be solved
C	X : (input/output) Real array of length NP containing the starting
...
C
C	SUBROUTINE FCN(NP,X,F) or SUBROUTINE FCN(NP,X,F,DF)
...
C
C	Required routines : BROYDN (or NEWTON), GAUELM, FCN
C
...
C	To initialise the parameterised function
      THETA=0.0
C	For NEWTON use the following form
C      CALL FCN(NP,X,F0,WK)
      CALL FCN(NP,X,F0)
      DO 500 I=1,NP
500   X0(I)=X(I)
...
C        CALL NEWTON(FCN,NP,X,F,REPS0,AEPS0,IER,WK,IWK)
...
C	-------------------
C
C	For use with NEWTON include DF
C
C	SUBROUTINE FCN(NP,X,F)
C	IMPLICIT REAL*8(A-H,O-Z)
C	PARAMETER (NMAX=200)
C	DIMENSION X(NP),F(NP)
...
C     SPECIFY THE SYSTEM OF NONLINEAR EQUATIONS  F(I)=0, FOR I=1,...,N
