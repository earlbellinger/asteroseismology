...
C	M : (input) Number of first order differential equations in the system
C	ML : (input) Number of boundary conditions at the first boundary t=T(1)
C	PAR : (input/output) Real array to be passed on to EQN, EQND, BCS and BCSD
...
C		not used by GEVP, but are merely used to pass on any extra parameters
C		that may be required to specify the equations
C	X : (output) Real array of length M*N containing the eigenvector.
C		X(i,j) is the ith component of solution at jth mesh point.
C		First dimension of X in calling program must be M
C	XC : (output) Real array of length M*N containing the left eigenvector.
...
C	BCSD : (input) Name of subroutine to calculate derivative of the boundary
C		conditions with respect to the eigenvalue
C		A sample outline for subroutines EQN, BCS, EQND, BCSD is
C		included at the end of this file.
...
C		F is a real array of length M which should be set to zero.
C		F, A and B must be calculated by the subroutine. T is the
C		value of t at which solution Y is specified.
C
...
C		BC is a real array of length (M+ML)*M which should contain
...
C		G is a real array of length M which should be set to zero.
...
C		to pass on any required parameters to define the equations.
C		PAR(1) is the eigenvalue.
C		A and B are real arrays of length (M+ML)*M which should
...
C		at t=T1, while remaining rows will specify those at t=TN.
C
C	Required routines : SETMAT, GAUBLK, MULER2 (or SECANI), EQN, EQND, BCS, BCSD
C
      SUBROUTINE GEVP(N,M,ML,PAR,X,XC,T,E0,EQN,BCS,EQND,BCSD,
     1                IWK,WK,IFLAG,REPS,RMX,IER)
      IMPLICIT REAL*8(A-H,O-Z)
C	For use with SECANI there is no need for complex variables
C	For MULER2 use the following two statements instead of the preceding one
...
C	First call to MULER2 or SECANI
C1000  CALL MULER2(CX1,CX2,CX3,REPS,RAPS,IER,CF,CX,IDET,NZ,CZERO,RMX)
C	For SECANI use the following statement. (CX, CF should be REAL)
