C     PROGRAM TO CALCULATE WEIGHTS AND ABSCISSAS OF GAUSS-JACOBI QUADRATURE FORMULAS
C     WITH WEIGHT FUNCTION (1-X)**ALP*(1+X)**BETA
...
C     USE THE WEIGHTS AND ABSCISSAS TO EVALUATE THE INTEGRAL OF X**N FOR TESTING
C     Only N=0,1,2 are used, so the test is not complete
...
C     To calculate weights and abscissas of a Gauss-Jacobi quadrature formula
C
C     N : (input) Number of points in the required quadrature formula
C     ALP : (input) Exponent of 1-x in weight function
C     BETA : (input) Exponent of 1+x in weight function
C		w(x)=(1-x)**ALP*(1+x)**BETA
C     W : (output) Array of length N, which will contain the weights
C     A : (output) Array of length N containing the abscissas
C     IER : (output) Error parameter, IER=0 implies successful execution
C		IER=302 implies N.LE.0
C		IER=313 implies ALP.LE.-1 or BETA.LE.-1
...
C     WK : Real array of length N*(N+2)+3*(N+1) used as scratch space
C
C     Required routines : GAUSRC, TQL2, GAMMA
...
C     Coefficients of recurrence relation
      WK(1,1)=1.D0+(ALP+BETA)/2.0
      WK(2,1)=(ALP-BETA)/2.0
C	WK(3,1) is not required and would give 0/0 form in some cases
C	Hence do not calculate it.
