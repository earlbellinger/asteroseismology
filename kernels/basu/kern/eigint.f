      subroutine eigint(x,y,rhs,ii,iy,ig,isn,nd1,nn)
c
...
c   
      common/clscon/ icls
c
      dimension x(nn),y(iy,nn),f(2,4),cc(2,20),al(2),a(2,2)
      external rhs
      ifd=2
      icls=0
...
c  right hand sides at next point
   10 nd=nd+isn
      ...
      dx=x2-x1
c
      call rhs(x2,f(1,if2),ifd,nd)
c  set mean coefficient matrix
      j1=if1-1
      j2=if2-1
      do 15 i=1,2
      do 15 j=1,2
   15 a(i,j)=0.5*(f(i,j1+j)+f(i,j2+j))
c
      do 17 i=1,2
   17 y(i,n)=y(i,n1)
...
c  increment icls by contribution from this interval
      icls=icls+iclcon(a,dx,al,cc,ie)
c
      if(nc.eq.nn) return
      i=if1
      if1=if2
...
c
    5 i=i+1
      ...
      integer function iclcon*4(a,tt,al,cc,ie)
...
c  find sign of a(1,2)
   50 isg=0
      if(a(1,2).ne.0) isg=sign(1.,a(1,2))
