...
c                      ****************************************
c
c     notes on precision:
c
c     on ibm computers (or other machines with small word length), one
c     may have that
c
c        y is real*(4*ipy)
c     dydx is real*(4*ipdy)
c
c     note, however, that the computation is performed in single precision.
c
c     however on the cray single precision should always be sufficient.
c     here the arguments ipy and ipdy must be 1.
c
c  .............................................................................
c
      real   x(1)
...
c     test size of intervals
      ...
      c=hn2*hn3*(hn2*((hn1+h3)*(hn3+1.)-2.*h3-hn1*hn3)-
     .  (h3*h3+hn1*hn3))
      if(second) go to 34
   32 d=hn2*hn2
      dysc=hnin
...
c     zero interval. write diagnostics
  361 nj1=nx1+1
      nj2=nx1+4
      write(6,1000) nj1,nj2,(x(nx1+j),j=1,4)
      dydx(i)=0.
c
...
c     test size of intervals
      ...
      nj1=n-1
      nj2=n+1
      write(6,1000) nj1,nj2,x(nj1),x(n),x(nj2)
      go to 42
c
c  rescale differences
c
