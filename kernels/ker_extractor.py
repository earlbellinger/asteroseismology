...
from kertools import load_fgong, kernel
...
def parse_dir(directory, freqs_fname, fgong_fname, 
    ...
    for filename in list(os.walk(directory))[0][2]:
        if filename[-4:] == '.dat':
            ...
            for (var1, var2) in ( ('c', 'rho'),
                                  ('c^2', 'rho' ),
                                  ('u', 'Y') ):
                K1, K2 = kernel(var1, var2, ell, nu, eig, fgong)
                save(x, K1, K2, R, var1, var2, ell, n,
                    normalize, output_dir, save_dat, save_plot)
            
            #K_c_rho, K_rho_c = kernel('c', 'rho', ell, nu, eig, fgong)
            #K_c2_rho, K_rho_c2 = kernel('c^2', 'rho', ell, nu, eig, fgong)
            
            #save(x, K_c_rho, K_rho_c, R, "c", "\\rho", ell, n,
            #    normalize, output_dir, save_dat, save_plot)
            #save(x, K_c2_rho, K_rho_c2, R, "c^2", "\\rho", ell, n,
            #    normalize, output_dir, save_dat, save_plot)
...
def save(x, K1, K2, R, var1, var2, l, n, normalize, 
    ...
    if save_plot:
        ...
        plt.plot(x, R*K1, 'r-', label="$RK_{%s,%s}$"%(var1, var2))
        plt.plot(x, R*K2, 'b-', label="$RK_{%s,%s}$"%(var2, var1))
        plt.xlabel(r"r/R")
