...
if [ -z ${Y+x} ]; then Y=0.26483782; fi
if [ -z ${Z+x} ]; then Z=0.0164794; fi
if [ -z ${alpha+x} ]; then alpha=1.69746522; fi
if [ -z ${overshoot+x} ]; then overshoot=0.23458746; fi
if [ -z ${step+x} ]; then step=1; fi
...
set_params() { 
    ...
    if (( $(echo "$overshoot > 0" | bc -l) )); then
        if (( $(echo "$step > 0" | bc -l) )); then 
            ...
            change_param 'overshoot_f_above_nonburn_core' "$overshoot" "$inlist"
            change_param 'overshoot_f_above_nonburn_shell' "$overshoot" "$inlist"
            change_param 'overshoot_f_below_nonburn_shell' "$overshoot" "$inlist"
            change_param 'overshoot_f_above_burn_h_core' "$overshoot" "$inlist"
            change_param 'overshoot_f_above_burn_h_shell' "$overshoot" "$inlist"
            change_param 'overshoot_f_below_burn_h_shell' "$overshoot" "$inlist"
        fi
